...
export default function StageClearOverlay({ gameState, setGameState, addNotification }: Props) {
  ...
  useEffect(() => {
    ...
    setOptions(pool.sort(() => Math.random() - 0.5).slice(0, 3));
  }, []);
  ...
  const pickOption = (opt: RogueliteOption) => {
    setGameState(prev => {
      ...
      return {
        ...
        state: 'PLAYING',
        stage: prev.stage + 1,
        mobsTotal: Math.floor(10 + (prev.stage + 1) * 2),
        mobsKilled: 0,
        bossSpawned: false,
        player: {
          ...prev.player,
  ...
  const gachaWeapon = () => {
    if (gameState.gold >= 300) {
      // In a real app, we'd trigger the drop logic here. 
      // For simplicity in this UI, we'll just subtract gold and let the next stage drop handle it or similar.
      // Better: simulate a drop now.
  ...
  return (
    <motion.div 
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="absolute inset-0 bg-black/40 backdrop-blur-sm z-[300] flex flex-col items-center justify-center p-6 bg-[radial-gradient(circle_at_center,_#1a1a24_0%,_#050508_100%)] overflow-hidden"
    >
      {/* Background Decorations */}
      <div className="absolute top-20 left-40 opacity-10 text-6xl">🎋</div>
      <div className="absolute bottom-40 right-60 opacity-10 text-5xl">🪨</div>
      ...
      <motion.h1 
        ...
        className="text-gold font-serif text-4xl italic mb-2 tracking-wide drop-shadow-lg"
      >
        Kỳ Ngộ Giang Hồ
      </motion.h1>
      <motion.p 
        ...
        className="text-gray-500 text-[10px] mb-16 uppercase tracking-[0.4em] font-bold"
      >
        Chọn một cơ duyên để tiếp tục hành trình
      </motion.p>

      <div className="flex flex-col md:flex-row items-center md:items-end justify-center gap-4 md:gap-6 w-full mb-8 md:mb-16">
        {options.map((opt, i) => {
          const isMiddle = i === 1;
          return (
             <motion.div
              ...
              whileHover={{ y: -10, borderColor: '#d4af37', boxShadow: '0 0 30px rgba(212,175,55,0.1)' }}
              onClick={() => pickOption(opt)}
              className={`group flex flex-col items-center text-center p-6 md:p-8 rounded-lg cursor-pointer transition-all relative
                ${isMiddle 
                  ? 'w-full max-w-[280px] md:w-64 h-auto md:h-[28rem] bg-[#1a1a24] border-2 border-gold shadow-[0_0_50px_rgba(212,175,55,0.2)] md:z-10' 
                  : 'w-full max-w-[280px] md:w-56 h-auto md:h-80 bg-[#121218] border border-gray-800 hover:border-gold'}`}
            >
              {isMiddle && (
                <div className="absolute -top-3 bg-gold text-black text-[10px] px-3 py-1 font-bold uppercase tracking-tighter">
                  Quý Hiếm
                </div>
              )}
              
              <div className={`rounded-full flex items-center justify-center border aspect-square mb-6 transition-transform group-hover:scale-110
                ${isMiddle ? 'w-20 h-20 bg-blue-950/30 border-blue-900/50 text-4xl' : 'w-16 h-16 bg-gray-900/30 border-gray-800 text-3xl'}`}
              >
                <opt.icon className={`${isMiddle ? 'w-10 h-10' : 'w-8 h-8'} text-blue-400`} />
              </div>

              <h3 className={`font-serif mb-4 drop-shadow-md ${isMiddle ? 'text-blue-400 text-2xl' : 'text-gray-300 text-xl'}`}>
                {opt.name}
              </h3>
              
              <p className={`leading-relaxed font-serif ${isMiddle ? 'text-sm text-gray-400' : 'text-xs text-gray-500'}`}>
                {opt.desc}
              </p>
              
              <div className="mt-auto">
                <span className={`text-[9px] font-bold uppercase tracking-[0.2em] ${isMiddle ? 'text-blue-500' : 'text-gray-600'}`}>
