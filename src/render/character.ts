 ...
 *
 * Draws the player / mob / boss as a sprite-with-overlay (sect tint,
 * weapon, hands, optional cloak wings). Caller is responsible for
 * passing in the appropriate sprite for the current entity — this
 * module does not know about React refs or game state shape.
 *
...
export interface DrawHumanParams {
  ...
  time: number;
  hasCloak?: boolean;
  /** Pre-resolved sect id (e.g. "sl", "vd", or "" for non-sect mobs). */
  sectId: string;
  /** Pre-resolved sprite for this entity (sect-specific, fallback, mob, or boss). May be null. */
  sprite: CanvasImageSource | null;
}

export function drawHuman(ctx: CanvasRenderingContext2D, params: DrawHumanParams): void {
  const { x, y, sz, c, facing, isBoss, moving, time, hasCloak = false, sectId, sprite } = params;
  ...
  const bodyBounce = moving ? Math.abs(Math.sin(time * 0.015)) * sz * 0.1 : 0;
  ...
  if (sprite) {
    ...
    ctx.drawImage(sprite, -spriteW / 2, -spriteH * 0.85, spriteW, spriteH);
  ...
  ctx.save();
  const armPhase = moving ? Math.sin(time * 0.015) * sz * 0.4 : 0;
